...
app.post('/twilio-voice-webhook', (req, res) => {
  console.log('[Twilio Webhook] Incoming request body:', req.body);

  const CallSid = req.body.CallSid;
  if (!CallSid) {
    console.error('[Twilio Webhook] Missing CallSid!');
  ...
  console.log(`[Twilio Webhook] Responding with TwiML for CallSid=${CallSid}`);
...
server.on('upgrade', (request, socket, head) => {
  ...
  if (!callSid) {
    console.error('[WS Upgrade] Missing callSid in URL');
    socket.destroy();
    return;
  }
